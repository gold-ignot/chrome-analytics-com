...
export const metadata: Metadata = generateMetadata(generateFilterSEO('popular'));
...
export default function PopularExtensionsPage() {
  ...
  useEffect(() => {
    if (total > 0) {
      const seoData = generateFilterSEO('popular', total);
  ...
  const fetchExtensions = async () => {
    try {
      ...
      if (searchQuery.trim()) {
        ...
        response = await apiClient.getExtensions(currentPage, limit, 'users', 'desc');
      }
      ...
      <section className="bg-white border-b border-slate-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center">
          ...
          <p className="text-lg text-slate-600 mb-8 max-w-2xl mx-auto">
            Discover the Chrome extensions with the most users and highest adoption rates
          </p>
          ...
          <div className="max-w-lg mx-auto">
            <SearchBar 
              ...
              placeholder="Search popular extensions..." 
            />
          </div>
        </div>
      </section>
              ...
              {loading ? (
                ...
                <>
                  {isSearching ? (
                    ...
                    <>{total} extension{total !== 1 ? 's' : ''} sorted by popularity</>
                  )}
