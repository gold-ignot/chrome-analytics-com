...
func main() {
	...
	config.AllowHeaders = []string{"Origin", "Content-Length", "Content-Type", "Authorization", "X-Admin-Password", "x-admin-password", "Cache-Control", "Pragma", "Expires"}
	...
	api := router.Group("/api")
	{
		...
		automation := api.Group("/automation")
		{
			...
			automation.GET("/failed/stats", automationHandler.GetFailedJobsStats)
			automation.GET("/proxy/stats", automationHandler.GetProxyStats)
			automation.GET("/scraper/diagnostics", automationHandler.GetScraperDiagnostics)
